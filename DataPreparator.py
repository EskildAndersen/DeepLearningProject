...
class ImageDataset(torch.utils.data.Dataset):
    def __init__(self, annotations_file, img_dir, transform=None, target_transform=None):
        self.img_labels = pd.read_csv(os.getcwd(
        ) + annotations_file, sep='#\d+\t', header=None, names=['path', 'text']).reset_index(drop=True)
        self.img_dir = img_dir

        # Resize all pictures into the same dimensions
        self.transform = torchvision.transforms.Resize([300, 300])
    ...
    def __getitem__(self, idx):
        ...
        image = read_image(os.getcwd() + img_path)
...
#texts = pd.read_csv(os.getcwd() + txtFilePath, sep= '\t', header =None, names = ['path', 'text']).reset_index(drop = True)
